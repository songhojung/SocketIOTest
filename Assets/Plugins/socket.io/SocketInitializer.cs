    ...
    public class SocketInitializer : MonoBehaviour {
        ...
        IEnumerator InitCore(UniRx.IObserver<Socket> observer, UniRx.CancellationToken cancelToken) {
            ...
            if (webSocketTrigger == null || !webSocketTrigger.IsConnected) {
                ...
                int last = www.text.LastIndexOf('}') +1;
                //char[] str = www.text.ToCharArray();
                
                string newWWWdata = www.text.Remove(last, www.text.Length - last);
                // char strChar = str[98];
                if (textIndex != -1) {
                    var json = newWWWdata.Substring(textIndex);
